...
# gemma-4-12B-offsec-expert (GGUF)

**Base `gemma-4-12B-it` with an *Offensive Security & Reverse Engineering* FactBank baked into its chat-template.** The model answers
...
| | base 12B | **this model** | Δ | error-closure* |
|---|---|---|---|---|
| /44 landmine questions | 12/44 (27.3%) | **43/44 (97.7%)** | **+31** | 97% |
...
```bash
llama-server -m gemma-4-12B-offsec-expert-Q4_0.gguf --jinja --port 8080 --ctx-size 8192
```
...
## Provenance & license
- **Base:** `lmstudio-community/gemma-4-12B-it-QAT-GGUF` (Q4_0). This model = that GGUF with `tokenizer.chat_template` rewritten to embed an
