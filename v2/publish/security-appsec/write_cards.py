#!/usr/bin/env python3
"""Write the 3 HF model cards (E2B/12B/26B), each documenting BOTH editions (thinking-off + thinking-on)
with clear notes, the working sampling + context settings, and run instructions."""
import os
HERE = os.path.dirname(os.path.abspath(__file__))

SIZES = {
    "E2B": dict(quant="Q4_K_M", base="lmstudio-community/gemma-4-E2B-it-GGUF", eff="~2B (E2B)",
                gb="3.4 GB", hub="gemma-4-e2b-security-expert", bench=True),
    "12B": dict(quant="Q4_0", base="lmstudio-community/gemma-4-12B-it-QAT-GGUF", eff="12B",
                gb="6.98 GB", hub="gemma-4-12b-security-expert", bench=True),
    "26B-A4B": dict(quant="Q4_0", base="lmstudio-community/gemma-4-26B-A4B-it-QAT-GGUF", eff="26B (A4B MoE)",
                    gb="14.4 GB", hub="gemma-4-26b-security-expert", bench=True),
}

BENCH = """## How the three sizes compare — and vs a cloud model (hand-scored)
...
def card(size):
    c = SIZES[size]
    q = c["quant"]
    off = f"gemma-4-{size}-security-expert-{q}.gguf"
    on = f"gemma-4-{size}-security-expert-thinking-{q}.gguf"
...
# gemma-4-{size}-security-expert (GGUF)

**A {c['eff']} model that writes *secure code by default* — an application-security *FactBank* is baked into
...
|---|---|---|---|
| **thinking-OFF** (default) | `{off}` | production, speed, reliability | 0 empty answers, fast, secure. No reasoning trace. **The safe default.** |
| **thinking-ON** | `{on}` | you want a visible reasoning trace | Shows its work and is strong on most tasks, **but ~10% of the hardest-reasoning prompts can run long and return a *blank* answer** (fails safe — it does not emit insecure code; just retry). Needs a large context + generous output budget. |
...
{BENCH if c['bench'] else ''}## How to run
...
- **thinking-OFF:** https://lmstudio.ai/mhndayesh/{c['hub']}
- **thinking-ON:** https://lmstudio.ai/mhndayesh/{c['hub']}-thinking

### llama.cpp / any OpenAI-compatible server (this GGUF, full bank embedded)
```bash
# thinking-OFF (recommended)
llama-server -m {off} --jinja --ctx-size 32768 --port 8080
# thinking-ON
llama-server -m {on} --jinja --ctx-size 32768 --port 8080
```
...
## Provenance & license
- **Base:** `{c['base']}` (weights untouched). Only `tokenizer.chat_template` is rewritten to embed an
...
for size in SIZES:
    d = os.path.join(HERE, f"gemma-4-{size}-security-expert-GGUF")
    os.makedirs(d, exist_ok=True)
    open(os.path.join(d, "README.md"), "w", encoding="utf-8", newline="\n").write(card(size))
    print("wrote card", size)
