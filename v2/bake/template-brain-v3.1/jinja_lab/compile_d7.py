...
def compile_template(bank, sample=None):
    # curated first: provenance becomes "index < NCUR", costing zero bytes
    bank = sorted(bank, key=lambda f: (not f["curated"],))
    idx = build_index_v5(bank)
    ncur = sum(1 for f in bank if f["curated"])
    remap, post, txt, lib = {}, collections.defaultdict(list), [], []
    for i, f in enumerate(bank):
        remap[idx["short"][f["id"]]] = str(i)
        txt.append(f["txt"])
        lib.append(f["lib"].strip())
    ...
    tpl = (HEAD.replace("__NCUR__", str(ncur))
               .replace("__POST__", json.dumps(post, ensure_ascii=False))
