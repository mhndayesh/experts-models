# BCC to libbpf conversion guide (Andrii Nakryiko)
...
section on using BPF subprograms to reflect new libbpf
features added since the original publication.

## Why libbpf and BPF CO-RE?

Historically, BCC was a framework of choice
...
```c
    ...
    rlimit rlim = {
        .rlim_cur = 512UL When something doesn’t work as expected, the best way to start investigating
...
beyond the scope of this document, existing kernel selftests
and BCC libbpf-tools examples
...
- `__open()` – creates and opens BPF application;

- `__load()` – instantiates, loads, and verifies BPF application parts;

- `__attach()` – attaches all auto-attachable BPF programs (it’s
optional, you can have more control by using libbpf APIs directly);

- `__destroy()` – detaches all BPF programs and frees up all used
resources.
...
those `#include
- `), instead include a single `vmlinux.h` and
...
#include        /* most used helpers: SEC, __always_inline, etc */
#include      /* for BPF CO-RE helpers */
#include        /* for getting kprobe arguments */
#endif /* __BCC__ */
```
...
- `kprobe/` for kprobe and `kretprobe/` for kretprobe;

- `raw_tp/` for raw tracepoint;
...
`tracepoint____` naming pattern for tracepoint
“/”. BCC auto-generates corresponding type during runtime
...
be named `trace_event_raw_`, but sometimes few tracepoints in kernel
...
the conversion is straightforward: `data_loc_` becomes
`__data_loc_` (notice double underscores).
...
```c
#define KERNEL_VERSION(a, b, c) (((a) Similarly to getting the kernel version, you can extract any `CONFIG_xxx`
value from Kconfig:
...
can do that using a helper `bpf_core_field_exists()`, which will return
...
layouts (see “BPF Portability and CO-RE”
...
a `const volatile` global variable (for mutable ones, just drop `const volatile` qualifiers):
...
- if you are defining a mutable (non-`const`) variable, make sure they are
...
```c
#pragma unroll
for (i = 0; i Without loop unrolling or if the loop doesn’t terminate within fixed amount of
...
4.16, you'll have to mark such function as alway inlined with `static __always_inline`, so that BPF verifier will see them as a single big function:
...
up to 3 arguments. It's simple and easy to use, but it's quite
expensive, making it unsuitable to be used in production. So it's mostly
...
start. See BCC PR that added first
...
list is the best place to ask
questions and send bug reports about BPF, libbpf, and CO-RE in general. BCC
tools-related issues are best routed to BCC
project itself. Have fun!

---

# APPENDED SEED: BCC to libbpf + CO-RE conversion (curated old->new differences)
