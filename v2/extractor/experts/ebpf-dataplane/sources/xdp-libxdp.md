# XDP / libxdp / AF_XDP — API migration source (verbatim collation)

This file collates verbatim source text documenting the migration of XDP and AF_XDP
APIs out of libbpf into libxdp at the libbpf 1.0 / libxdp 1.0 boundary, plus the
netlink XDP-attach function renames (bpf_set_link_xdp_fd/bpf_get_link_xdp_id/...
-> bpf_xdp_attach/bpf_xdp_query_id/...).

================================================================================
## SOURCE 1 — xdp-tools/README.org (verbatim, master)
================================================================================
...
================================================================================
## SOURCE 2 — xdp-tools/lib/libxdp/README.org (verbatim, master)
================================================================================
...
================================================================================
## SOURCE 3 — libbpf 1.0 announcement (nakryiko.com/posts/libbpf-v1), relevant excerpts
================================================================================

> Other API clean ups, renames, removals, etc
> We used libbpf 1.0 milestone to clean up libbpf API surface significantly. We removed,
> renamed, or consolidated a good chunk of APIs:
> AF_XDP-related parts of libbpf (APIs from xsk.h) were consolidated into libxdp.

> Now, with bpf_tc_hook_create(), bpf_tc_hook_destroy(), bpf_tc_attach(), bpf_tc_detach(),
> and bpf_tc_query() APIs there is no need to take extra dependency on iproute2 just to
> attach BPF TC programs. All the batteries are included with libbpf. Similar in naming and
> spirit bpf_xdp_attach(), bpf_xdp_detach(), and bpf_xdp_query() APIs abstract away
> netlink-based XDP attachment details. Note that there is also a safer-by-default
> alternative bpf_link-based XDP attachment API, bpf_program__attach_xdp(), which is a
> preferred way of performing XDP attachment on newer kernels.

> Libbpf 1.0 marks the release in which all the deprecated APIs and features are physically
> removed from the code base and new stricter behaviors are mandatory and not an opt-in anymore.

================================================================================
## SOURCE 4 — libbpf.h netlink XDP-attach renames (verbatim, libbpf v0.8.0 deprecation comments)
================================================================================

The old netlink XDP-attach functions were deprecated in libbpf 0.8 and REMOVED in 1.0.
The deprecation comments state the exact replacement for each old function:
...
Old -> new mapping (from the LIBBPF_DEPRECATED_SINCE comments above):
- bpf_set_link_xdp_fd()      -> use bpf_xdp_attach() instead
- bpf_set_link_xdp_fd_opts() -> use bpf_xdp_attach() instead
- bpf_get_link_xdp_id()      -> use bpf_xdp_query_id() instead
- bpf_get_link_xdp_info()    -> use bpf_xdp_query() instead

================================================================================
## SOURCE 5 — xsk.h AF_XDP deprecation/move to libxdp (verbatim, libbpf v0.8.0)
================================================================================
...
Every symbol in xsk.h carries: LIBBPF_DEPRECATED_SINCE(0, 7, "AF_XDP support deprecated
and moved to libxdp"). The header-level note: "This whole API has been deprecated and moved
to libxdp that can be found at https://github.com/xdp-project/xdp-tools. The APIs are exactly
the same so it should just be linking with libxdp instead of libbpf for this set of
functionality."
